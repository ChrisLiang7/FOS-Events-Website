...
User.find({
  ...
  if (doc.length == 0) User(defaultUser).save();
...
exports.postLogin = (req, res, next) => {
  ...
  if (!validator.isEmail(req.body.email)) validationErrors.push({ msg: '请输入有效邮箱。' });
  if (validator.isEmpty(req.body.password)) validationErrors.push({ msg: '密码不能为空。' });
...
exports.postSignup = (req, res, next) => {
  const validationErrors = [];
  if (!validator.isEmail(req.body.email)) validationErrors.push({ msg: '请输入有效邮箱。' });
  if (!validator.isLength(req.body.password, { min: 8 })) validationErrors.push({ msg: '密码必须8位以上' });
  if (req.body.password !== req.body.confirmPassword) validationErrors.push({ msg: '密码不对应' });
  ...
  User.findOne({ email: req.body.email }, (err, existingUser) => {
    if (err) { return next(err); }
    if (existingUser) {
      req.flash('errors', { msg: '此账号已经注册，请重新登陆.' });
      return res.redirect('/signup');
    }
    user.save((err) => {
      if (err) { return next(err); }
...
exports.postUpdatePassword = (req, res, next) => {
  ...
  User.findById(req.user.id, (err, user) => {
    ...
    user.save((err) => {
      if (err) { return next(err); }
      req.flash('success', { msg: '密码已经保存修改.' });
      res.redirect('/account');
    });
  });
};
